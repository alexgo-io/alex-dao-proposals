...
(define-public (execute (sender principal))
	(begin
		...
		(try! (contract-call? .key-alex-autoalex-v1 set-approved-contract .collateral-rebalancing-pool-v1 false))
		(try! (contract-call? .yield-alex-v1 set-approved-contract .collateral-rebalancing-pool-v1 false))		
		...
		(let 
			...
			(try! (contract-call? .collateral-rebalancing-pool-v1 set-approved-contract 'SP1A6F9ABHQMVP92GH7T9ZBF029T1WG3SHPNMKT0D))
			(try! (contract-call? .age000-governance-token mint-fixed (+ ytp-balance crp-balance) tx-sender))
			(try! (contract-call? .auto-alex transfer-fixed tx-sender .collateral-rebalancing-pool-v1 crp-to-hold none))
